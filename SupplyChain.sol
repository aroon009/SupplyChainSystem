...
contract supplyChain{
    ...
    uint32 public participant_id = 0;   // participent Id
    ...
    mapping(uint32=>uint32[]) public productTrack; // owenerships by Product ID(product_id) / Movement track for a product
    ...
    function newOwner(uint32 _user1Id, uint32 _user2Id, uint32 _prodId) onlyOwner (_prodId) public returns(bool){
        ...
        if(keccak256(abi.encodePacked(p1.participantType))== keccak256("Manufacture")
